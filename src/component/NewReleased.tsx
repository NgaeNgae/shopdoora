...
const NewReleased = () => {
  ...
  return (
    ...
    <div className='flex max-sm:block max-lg:flex-wrap max-sm:gap-10 max-lg:gap-y-7 gap-2'>
      {
                products.map((product,index) => {
                  return (
                    <div key={index} className="w-[25%] max-lg:w-[49%] shadow-lg transform active:scale-90 transition-transform max-sm:w-[90%] max-sm:my-5 max-sm:mx-auto relative rounded-lg">
                      <img className='rounded-xl' src={product.src} alt="" />
                      <div className='absolute max-lg:text-sm max-lg:w-[70%] bottom-0 w-[59%] p-4'>
                        <p>{product.name}</p>
